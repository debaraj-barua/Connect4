...
void playGame()
{
    ...
    ComputerPlayer maxPlayerX = ComputerPlayer(&fourGame, isMaxPlayer, (bool) pruning, depth);
           ...
           !fourGame.isFieldFull())
    {
        ...
        if (maxPlayersTurn) move = maxPlayerX.makeMove();
        else move = minPlayerO.makeMove();
        
        if (maxPlayersTurn) cout << "MaxPlayer chose column: " << move << "\n";
        else cout << "MinPlayer chose column: " << move << "\n";
    ...
    {
        if (utility == INT_MAX) cout << "Max Player won, congratulations";
        else if (utility == INT_MIN) cout << "Min Player won, congratulations";
        else cout << "ERROR: This should never be printed";
    }
}
